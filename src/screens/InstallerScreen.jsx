...
import { saveScannedProduct } from '../service/InstallerproductService';

const { width } = Dimensions.get('window');

const PRODUCT_MAP = {
  'SOLAR-429': { product: 'Solar Panel', orderId: '#429', status: 'Installed' },
  'SOLAR-430': { product: 'Solar Inverter', orderId: '#430', status: 'Installed' },
  'SOLAR-431': { product: 'Solar Battery', orderId: '#431', status: 'Installed' },
};
...
const InstallerScreen = ({ navigation }) => {
  ...
  const [trackingSteps, setTrackingSteps] = useState([
    ...
    { id: 2, label: 'Installed at Site', sub: '', time: '', done: false },
    { id: 3, label: 'Installation Verified', sub: '', time: '', done: false },
  ]);
    ...
    const loadUser = async () => {
      try {
        ...
        if (data) {
          ...
          setUserMobile(user?.UserInfo?.phoneNo || user?.phoneNo || user?.mobile);
        }
      } catch (e) {
        console.log('[Installer] User load error:', e);
      }
    };
    loadUser();
  }, []);
  ...
  const restoreState = async () => {
    try {
      ...
      console.log('[InstallerScreen] restoreState error:', e);
    }
  };
  ...
  const codeScanner = useCodeScanner({
    ...
    onCodeScanned: useCallback((codes) => {
      ...
      setIsScanning(false);
      const found = PRODUCT_MAP[value] ?? { product: value, orderId: '#???', status: 'Installed' };
  ...
  const updateToInstalled = () => {
    ...
    setTrackingSteps([
      ...
      },
      {
        ...
        sub: `Location: ${locationRef.current ? `${locationRef.current.latitude.toFixed(4)}, ${locationRef.current.longitude.toFixed(4)}` : 'Site Location'}`,
        time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        done: true,
      },
      {
    ...
    setTrackingSteps((prev) =>
      prev.map((step) =>
        step.id === 3
          ? {
              ...step,
              sub: `Verified at ${locationRef.current ? `${locationRef.current.latitude.toFixed(4)}, ${locationRef.current.longitude.toFixed(4)}` : 'Site'}`,
              time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
              done: true,
            }
          : step
      ...
      <View style={{ flex: 1, backgroundColor: '#fff' }}>
        ...
        <SafeAreaView style={{ flex: 1 }} edges={['top']}>
          ...
          <LinearGradient colors={['#F00001', '#B00100']} style={styles.logoBanner}>
            <Image source={require('../../assets/images/kondass.png')} style={{ width: 160, height: 70 }} resizeMode="contain" />
          </LinearGradient>
          <View style={styles.welcomeContainer}>
            <Text style={styles.welcomeText}>Welcome!</Text>
            <Text style={styles.welcomeSub}>Turn on availability to start installations</Text>
          </View>
        </SafeAreaView>
      </View>
    );
                ...
                <View style={styles.card}>
                  <View style={styles.rowBetween}>
                    ...
                    <View style={styles.scannedBadge}>
                      ...
                      <Text style={styles.scannedText}>Installed</Text>
                    </View>
                  </View>
                </View>
              )}
          ...
          {currentView === 'tracking' && (
            <>
              ...
              {scannedProducts.length > 0 && (
                <View style={styles.card}>
                  <Text style={styles.sectionLabel}>INSTALLED PRODUCTS</Text>
                  {scannedProducts.map((item, index) => (
                    <View key={index} style={styles.scannedItem}>
                      <MaterialCommunityIcons name="solar-panel" size={24} color="#3b82f6" />
                      <View style={{ marginLeft: 12, flex: 1 }}>
                        <Text style={{ fontWeight: '600' }}>{item.product}</Text>
                        <Text style={{ fontSize: 12, color: '#64748b' }}>{item.orderId}</Text>
                      </View>
              ...
              <Text style={styles.sectionLabel}>INSTALLATION PROGRESS</Text>
              <View style={styles.card}>
                ...
                {!isCompleted && (
                  <TouchableOpacity style={styles.deliverBtn} onPress={markAsVerified}>
                    <Ionicons name="shield-check" size={18} color="#fff" />
                    <Text style={styles.deliverBtnText}>Mark as Verified</Text>
                  </TouchableOpacity>
                )}
              </View>
              ...
              {locationRef.current && (
                <View style={styles.liveLocationCard}>
                  ...
                  <View style={{ marginLeft: 10 }}>
                    <Text style={styles.liveLocLabel}>Current Site</Text>
      ...
      <Modal transparent visible={showLogoutPopup} animationType="fade" onRequestClose={() => setShowLogoutPopup(false)}>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 24 }}>
          <View style={{ backgroundColor: '#fff', borderRadius: 16, padding: 24, width: '100%' }}>
            <Text style={{ fontSize: 18, fontWeight: '700', color: '#1a1a1a', marginBottom: 8 }}>Logout</Text>
            <Text style={{ fontSize: 14, color: '#64748b', marginBottom: 24 }}>Are you sure you want to logout?</Text>
            <View style={{ flexDirection: 'row', gap: 12 }}>
              <TouchableOpacity onPress={() => setShowLogoutPopup(false)} style={{ flex: 1, paddingVertical: 12, borderRadius: 10, borderWidth: 1, borderColor: '#e2e8f0', alignItems: 'center' }}>
                <Text style={{ fontWeight: '600', color: '#64748b' }}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleLogout} style={{ flex: 1, paddingVertical: 12, borderRadius: 10, backgroundColor: '#F00001', alignItems: 'center' }}>
...
const styles = StyleSheet.create({
  ...
  scannedBadge:          { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: '#fef9c3', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 20 },
  scannedText:           { fontSize: 11, color: '#f59e0b', fontWeight: '600' },
  ...
  stepSub:               { fontSize: 12, color: '#64748b', marginTop: 2, marginBottom: 16 },
  actionBtn:             { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 13, borderRadius: 10 },
  actionBtnText:         { color: '#fff', fontWeight: '700', fontSize: 14 },
  hintBox:               { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#f1f5f9', padding: 12, borderRadius: 10 },
  hintText:              { fontSize: 13, color: '#64748b', flex: 1 },
  ...
  input:                 { borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 10, paddingHorizontal: 14, paddingVertical: 11, fontSize: 14, color: '#1a1a1a', marginBottom: 12, backgroundColor: '#f8fafc' },
  ...
  modalBtnCancel:        { flex: 1, paddingVertical: 12, borderRadius: 10, borderWidth: 1, borderColor: '#e2e8f0', alignItems: 'center' },
  modalBtnConfirm:       { flex: 1, flexDirection: 'row', paddingVertical: 12, borderRadius: 10, backgroundColor: '#F00001', alignItems: 'center', justifyContent: 'center' },
});
