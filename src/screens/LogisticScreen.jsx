...
const PRODUCT_MAP = {
  'SOLAR-429': { product: 'Solar Panel', orderId: '#429', status: 'Scanned' },
  'SOLAR-430': { product: 'Solar Inverter', orderId: '#430', status: 'Scanned' },
  'SOLAR-431': { product: 'Solar Battery', orderId: '#431', status: 'Scanned' },
};
...
const LogisticScreen = ({ navigation }) => {
  ...
  const codeScanner = useCodeScanner({
    ...
    onCodeScanned: useCallback((codes) => {
      ...
      setIsScanning(false);
      const found = PRODUCT_MAP[value] ?? { product: value, orderId: '#???', status: 'Scanned' };
  ...
  const updateToPickedFromWarehouse = () => {
    ...
    setTrackingSteps([
      { id: 1, label: 'Product Scanned', sub: `${total} Product${total > 1 ? 's' : ''} Scanned`, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), done: true },
      { id: 2, label: 'Picked from Warehouse', sub: `Location: ${currentLocation ? `${currentLocation.latitude.toFixed(4)}, ${currentLocation.longitude.toFixed(4)}` : 'Chennai Warehouse'}`, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), done: true },
      { id: 3, label: 'Delivered to Warehouse', sub: 'Waiting to reach drop location...', time: '', done: false },
    ]);
  };
      ...
      <View style={{ flex: 1, backgroundColor: '#fff' }}>
        ...
        <SafeAreaView style={{ flex: 1 }} edges={['top']}>
          ...
          <View style={styles.profileRow}>
            ...
            <Switch value={isAvailable} onValueChange={handleToggle} trackColor={{ false: '#ccc', true: '#F00001' }} thumbColor="#fff" />
          </View>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <View style={styles.topBar}>
          ...
          <View style={styles.profileRowSmall}>
            ...
            <Switch value={isAvailable} onValueChange={handleToggle} trackColor={{ false: '#ccc', true: '#F00001' }} thumbColor="#fff" style={{ marginLeft: 12 }} />
          </View>
        </View>

        <View style={styles.tabRow}>
          {['scanner', 'tracking'].map((tab) => (
            <TouchableOpacity key={tab} style={[styles.tabBtn, currentView === tab && styles.tabBtnActive]} onPress={() => setCurrentView(tab)}>
            ...
            <>
              ...
              {locationRef.current && (
                <View style={styles.locationBadge}>
                  ...
                  <Text style={styles.locationBadgeText}>{locationRef.current.latitude.toFixed(5)}, {locationRef.current.longitude.toFixed(5)}</Text>
                </View>
              )}
                ...
                {trackingSteps.map((step, index) => (
                  <View key={step.id} style={styles.stepRow}>
                    <View style={styles.stepLeft}>
                      ...
                      {index < trackingSteps.length - 1 && <View style={[styles.stepLine, step.done && styles.stepLineDone]} />}
                    </View>
